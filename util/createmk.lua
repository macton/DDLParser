...
Usage: lua createmk.lua <platform>, where <platform> can be mingw,
linux, msvc-x86 or msvc-x64.
...
if platform == 'mingw' then
  ...
  config.CC     = 'g++ -O2 -Iinclude -D__WIN__ -Wall -Wno-format -o $@ -c %1'
  config.CCLIB  = config.CC
  config.CCLIBD = 'g++ -O0 -g -Iinclude -D__WIN__ -Wall -Wno-format -o $@ -c %1'
  config.LIB    = 'ar cru $@ ${ALLDEPS} && ranlib $@'
  config.LINK   = 'g++ -o $@ ${ALLDEPS}'
...
elseif platform == 'linux' then
  ...
  config.CC     = 'g++ -O2 -Iinclude -Wall -Wno-format -o $@ -c %1'
  config.CCLIB  = config.CC
  config.CCLIBD = 'g++ -O0 -g -Iinclude -Wall -Wno-format -o $@ -c %1'
  config.LIB    = 'ar cru $@ ${ALLDEPS} && ranlib $@'
  config.LINK   = 'g++ -o $@ ${ALLDEPS}'
  config.GPERF  = 'gperf -c -C -l -L C++ -t -7 -m 100 -I'
  config.RM     = 'rm -f'
...
elseif platform == 'msvc-x64' then
  ...
  config.CC     = 'cl /Iinclude /Ilua /nologo /D__WIN__ /Dsnprintf=_snprintf /D_USE_MATH_DEFINES /DNDEBUG /Ox /GF /EHsc /MD /GS- /Gy /fp:precise /Zc:forScope /Gd /Fo$@ /c %1'
  config.CCLIB  = 'cl /Iinclude /Ilua /nologo /D__WIN__ /Dsnprintf=_snprintf /D_USE_MATH_DEFINES /DNDEBUG /Zi /Ox /GF /EHsc /MD /GS- /Gy /fp:precise /Zc:forScope /Gd /Fdoutput\\release\\ddlparser.pdb /Fo$@ /c %1'
  config.CCLIBD = 'cl /Iinclude /Ilua /nologo /D__WIN__ /Dsnprintf=_snprintf /D_USE_MATH_DEFINES /UNDEBUG /Zi /Od /EHsc /RTC1 /MDd /GS /fp:precise /Zc:forScope /Gd /Fdoutput\\debug\\ddlparser.pdb /Fo$@ /c %1'
...
.lt.h:
  util\text2c $< $(*F)_template > $@

all: output\debug\${LIB:ddlparser} output\release\${LIB:ddlparser} ddlt\ddlt${EXEEXT} test\test${EXEEXT}
...
util\text2c${OBJEXT}: ${DEPS:util\text2c.cpp}
  ${CC:util\text2c.cpp}

util\text2c${EXEEXT}: util\text2c${OBJEXT}
  ${LINK}
...
ddlt\ddlt${OBJEXT}: ${DEPS:ddlt\ddlt.cpp} $(LUAFILES)
  ${CC:ddlt\ddlt.cpp}

ddlt\extra.h: ddlt\extra.lua util\text2c${EXEEXT}

ddlt\ddlc.h: ddlt\ddlc.lua util\text2c${EXEEXT}

ddlt\cpp.h: ddlt\cpp.lt util\text2c${EXEEXT}

ddlt\hpp.h: ddlt\hpp.lt util\text2c${EXEEXT}
...
test\test_ddl.h: ddlt\ddlt${EXEEXT} test\test.ddl
  ddlt\ddlt -i test\test.ddl -t hpp -o test\test_ddl.h

test\test_ddl.cpp: ddlt\ddlt${EXEEXT} test\test.ddl
  ddlt\ddlt -i test\test.ddl -t cpp -I test_ddl.h -o test\test_ddl.cpp

test\test_ddl${OBJEXT}: test\test_ddl.cpp test\test_ddl.h
  ${CC:test\test_ddl.cpp}

test\test${OBJEXT}: ${DEPS:test\test.cpp} test\test_ddl.h
  ${CC:test\test.cpp}

test\test${EXEEXT}: test\test${OBJEXT} test\test_ddl${OBJEXT}
  ${LINK}
  echo "Running test..."
  test\test
...
clean:
  ${RM} output\debug\${LIB:ddlparser} $(LIBOBJSD) output\release\${LIB:ddlparser} $(LIBOBJSR) src\Tokens.inc ddlt\ddlt${EXEEXT} $(DDLTOBJS) etc\text2c${EXEEXT} etc\text2c${OBJEXT} $(LUAFILES) test\test_ddl.h test\test_ddl.cpp test\test_ddl${OBJEXT} test\test${OBJEXT} test\test${EXEEXT} util\text2c${EXEEXT} util\text2c${OBJEXT} output\debug\ddlparser.pdb output\release\ddlparser.pdb
]]
